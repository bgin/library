...
!                   
!
  call pakstr(ang_grid,len)     
  IF( ang_grid(1:len) == 'yes' ) THEN
      ...
      IF (nonsep == .true.) THEN
          ...
          DO i = 1, ang_loop
             ...
             IF ( dollar(str,card,cpass,inp) ) THEN  
                  ...
                  atom%shl(i)%ang%n_pts=atom%shl(i)%ang%leb_ang%nang
             ELSE
      ...
      END IF
  END IF 
  call pakstr(rad_grid,len)     
  IF( rad_grid(1:len) == 'yes' ) THEN
...
!
  ...
  END IF
  IF ( ang_grid == 'no' .or. rad_grid == 'no') THEN
       return
  END IF      
!
!  Now for each shell create the 3D grid
!
  CALL iosys('read character "non separable quadrature '//str//'" from grid',0,0,0,yes_no)
!  IF (yes_no == 'yes') THEN
!     Lebedev Quadrature
!     DO i = 1, atom%n_shl 
!        atom%shl(i)%n_3d = atom%rad%reg(i)%n_pts * atom%shl(i)%ang%n_pts
!        ALLOCATE(atom%shl(i)%grid(1:3,1:atom%shl(i)%n_3d))
!        Call Make_XYZ(atom%shl(i)%grid,atom%shl(i)%ang%leb_ang,atom%rad%reg(i))
!     END DO
!     Separable Quadrature
!     DO i = 1, atom%n_shl 
!        atom%shl(i)%n_3d = atom%rad%reg(i)%n_pts * atom%shl(i)%ang%n_pts
!        ALLOCATE(atom%shl(i)%grid(1:3,1:atom%shl(i)%n_3d))
!        Call Make_XYZ(atom%shl(i)%grid,atom%shl(i)%ang%theta_ang%reg,atom%shl(i)%ang%phi_ang%reg,  &
!                      atom%rad%reg(i))
!     END DO
