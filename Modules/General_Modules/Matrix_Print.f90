...
!***********************************************************************
!Deck Callprt
  Subroutine Callprt(mattyp,vec,mat,n,m,frmt,title,collab,rowlab)
  IMPLICIT NONE
  CHARACTER(LEN=*)                         :: mattyp
  REAL(idp), DIMENSION(:)                  :: vec
  ...
  IF ( mattyp == 'real_vector') THEN
       Call Print_Matrix(type_real_vector,vec,frmt,title,collab)
  ELSE IF( mattyp == 'real_matrix') THEN
       Call Print_Matrix(type_real_matrix,mat,n,m,frmt,title,collab,rowlab)
  ELSE
        Write(iout,*) '     Error in call'
  END IF
  end subroutine  Callprt
!deck Print_Matrix_d
...
1 format(18x,a8)
2 format(10x,f20.12)
3 format(4(f20.12,1x))
end subroutine  Print_Vector_d
!***********************************************************************
!***********************************************************************
!deck Print_Vector_i
  subroutine Print_Vector_i(type_integer_vector,a,title)
  implicit none
...
!
  ...
  IF (PRESENT(frmt)) THEN
      ...
      ELSE IF (frmt == 'fr') THEN
          IF ( l == 0 ) THEN
               write (iout,3) a(:)
          ELSE
               write (iout,3) a(1:a_size-1)
               write (iout,4) a(a_size)
          END IF
      END IF
  ELSE
      IF (PRESENT(collab)) THEN
  ...
  END IF
1 format(18x,a8)
2 format(10X,2f20.12)
3 format ( '('f20.12,','f20.12')','('f20.12,','f20.12')')
4 format ( '('f20.12,','f20.12')')
