...
!
  IF(spdim == 1) THEN
     CALL dvr_mat_mul_z(v_in,v_out,nphy(1),1,1)
     CALL v_diag_mul_1d_z(v_in,v_out)
  ELSE IF(spdim == 2) THEN
!
!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        CALL dvr_mat_mul_z(v_in,v_out,nphy(2),nphy(1),2)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!       This code is for the general problem involving nphy(1),
!       V(2,1) * H(1,1)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        CALL dvr_mat_mul_2d_z(v_in,v_out,nphy(2),nphy(1),1,1)
        CALL v_diag_mul_2d_z(v_in,v_out)
!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ELSE IF(spdim == 3) THEN
!
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        CALL dvr_mat_mul_z(v_in,v_out,nphy(3),nphy(2)*nphy(1),3)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!        This is for the general case involving the matrix multiply 
!        involving nphy(2), that is, V(3,2,1) * H(2,2)
!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        CALL dvr_mat_mul_2d_z(v_in,v_out,nphy(3),nphy(2),nphy(1),2)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!        This is for the general problem involving the matrix multiply 
!        for nphy(1), that is, V(3,2,1) * H(1,1)
!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        CALL dvr_mat_mul_2d_z(v_in,v_out,nphy(3)*nphy(2),nphy(1),1,1)
        CALL v_diag_mul_3d_z(v_in,v_out)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  END IF
...
!
  SUBROUTINE v_diag_mul_1d_z(v_in,v_out)
  USE dvr_shared
  USE dvr_Global
  IMPLICIT NONE
  COMPLEX*16, DIMENSION(nphy(1))          :: v_in
  COMPLEX*16, DIMENSION(nphy(1))          :: v_out
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  v_out(:) = v_out(:) + grid(1)%v(:) * v_in(:)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
  SUBROUTINE v_diag_mul_2d_z(v_in,v_out)
  USE dvr_shared
  USE dvr_Global
  IMPLICIT NONE
  COMPLEX*16, DIMENSION(nphy(2),nphy(1))     :: v_in
  COMPLEX*16, DIMENSION(nphy(2),nphy(1))     :: v_out
  INTEGER                                    :: i
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  DO i = 1, nphy(2)
     v_out(i,:) = v_out(i,:) + grid(2)%v(i) * v_in(i,:)
...
!
  SUBROUTINE v_diag_mul_3d_z(v_in,v_out)
  USE dvr_shared
  USE dvr_Global
  IMPLICIT NONE
  COMPLEX*16, DIMENSION(nphy(3),nphy(2),nphy(1))     :: v_in
  COMPLEX*16, DIMENSION(nphy(3),nphy(2),nphy(1))     :: v_out
  INTEGER                                            :: i
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  DO i = 1, nphy(3)
     v_out(i,:,:) = v_out(i,:,:) + grid(3)%v(i) * v_in(i,:,:)
...
!
   nk=size(dvr_mat,1)
   nj=size(v,2)
   Call ebc(v_scr,dvr_mat,v,nk,nk,nj)
!   DO i=1,nk
!      DO k=1,nk
...
!
   nk=size(dvr_mat,1)
   nj=size(v,2)
   Call ebcc(v_scr,dvr_mat,v,nk,nk,nj)
!   DO i=1,nk
!      DO k=1,nk
!         v_scr(k,:) = v_scr(k,:) + dvr_mat(k,i) * v(i,:) 
!      END DO
...
!
  ni=size(v,1)
  nk=size(dvr_mat,1)
  Call ebc(v_scr,v,dvr_mat,ni,nk,nk)
!  DO k=1,nk
!     DO j=1,nk
...
!
  nk=size(dvr_mat,1)
  ni=size(v,1)
  Call ebcc(v_scr,v,dvr_mat,ni,nk,nk)
!  DO k=1,nk
!     DO j=1,nk
!        v_scr(:,j) = v_scr(:,j) + v(:,k) * dvr_mat(k,j) 
!      END DO
...
!
   ...
   ni=size(v,2)
!   v_scr(1:nk,:) = 0.d0
   Call ebc(v_scr,dvr_mat,v,nk,nk,ni)
!   DO i=1,nk
!      DO k=1,nk
!         v_scr(k,:) = v_scr(k,:) + dvr_mat(k,i) * v(i,:) 
!      END DO
...
!  v_scr(1:nk,:) = (0.d0,0.d0)
  Call cebc(v_scr,dvr_mat,v,nk,nk,ni)
!  DO i=1,nk
!     DO k=1,nk
!        v_scr(k,:) = v_scr(k,:) + dvr_mat(k,i) * v(i,:)     
!     END DO
...
!
  ...
  ni=size(v,1)
!  v_scr(:,1:nk)=(0.d0,0.d0)
  Call cebc(v_scr,v,dvr_mat,ni,nk,nk)
!  DO k=1,nk
!     DO j=1,nk
!        v_scr(:,j) = v_scr(:,j) + v(:,k) * dvr_mat(k,j)
!     END DO
