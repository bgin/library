*deck @(#)matprt.f	5.1  11/6/94
      subroutine matprt(a,md,nd,nrows,ncols,lrow,lcol,rowlab,
     $                  collab,isym,eig,ifeig)
c***begin prologue     matprt.f
c***date written       yymmdd  
c***revision date      11/6/94      
c
c***keywords           
c***author             unknown
c***source             @(#)matprt.f	5.1   11/6/94
c***purpose            
c***description
c     
c    
...
c
      ...
      parameter (coldat=5)
c
 9000 format(15x,10i15)
c 9010 format(5x,'eigenvalues -- ',1x,10f10.5)
 9010 format(5x,'eigenvalues --  ',5e15.8)
 9020 format(1x,a80)
 9030 format(1x,i9,11x,5e15.8)
 9040 format(1x,10x,a8,2x,5e15.8)
 9050 format(1x,2x,a16,2x,5e15.8)
 9060 format(1x,4x,i3,2x,a8,3x,5e15.8)
 9070 format(1x,i3,1x,a16,5e15.8)
c
...
c        write eigenvalues if requested.
         if(ifeig) then
            write(iout,9010) (eig(i),i=colmin,colmax)
            write(iout,*)
         endif
c
...
c           write the row.
            if(lrow.eq.0) then
               ...
               if(rowwid.eq.8)  write(iout,9040) line,
     $                               (a(row,j),j=colmin,collim)
               if(rowwid.eq.16) write(iout,9050) line,
     $                               (a(row,j),j=colmin,collim)
            else if(lrow.eq.2) then
               if(rowwid.eq.8)  write(iout,9060) line,
     $                               (a(row,j),j=colmin,collim)
               if(rowwid.eq.16) write(iout,9070) line,
     $                               (a(row,j),j=colmin,collim)
            endif
   90    continue
  100 continue
